<?php
  ...
  $ratings = $db->query("SELECT AVG(rate) AS ave_rating, COUNT(id) as reviewers FROM trainor_ratings_db WHERE trainor_id = '$my_id' AND delete_flg = '0';");
  ...
  <div class="content-wrapper">
    <div class="container">
            ...
            <div class="d-sm-flex mt-3 mb-5">
                  <?php if(file_exists("../uploads/profile/trainor/".$profile_data) && $profile_data != null): ?>
                    <img src="../uploads/profile/trainor/<?=$profile_data?>" alt="User profile picture"
              ...
              ?>
                ...
                <div class="d-flex justify-content-center">
                  <h3 class="text-bold"><?php 
                      ...
                      for ($i=0; $i < round($ratings_row['ave_rating'], 2) ; $i++) { 
                    ?>
            ...
            <?php
              ...
              from_reg.user_type_id,
              review.*
              FROM trainor_ratings_db review
              JOIN reg_db from_reg
              ON review.reg_id = from_reg.id
              WHERE review.delete_flg = '0'
              AND review.trainor_id = '$my_id';");                                                                                                                                                                                                                                                                                                                                                                                

              while ($reviews = mysqli_fetch_assoc($query)):
                $profile_picture = $reviews['profile_picture'];
                $user_type = $reviews['user_type_id'];
                $user = "";
                if($user_type == "1"){
                  $user = "client";
                }else if($user_type == "2"){
            ...
            ?>
              ...
              <div class="mb-5">
                <div class="d-flex mt-3">
                  <?php if(file_exists("../uploads/profile/$user/".$profile_picture) && $profile_picture != null): ?>
                    <img src="../uploads/profile/<?= $user ?>/<?=$profile_picture?>" alt="User profile picture"
                    ...
                    <div class="d-flex justify-content-between">
                      <h6 class="text-bold">
                        <?php 
                          ...
                          for ($i=0; $i < round($reviews['rate'], 0) ; $i++) { 
                        ?>
                            <i class="fa fa-star text-warning"></i>
                        <?php
                          }
                      ...
                      <h6 class="ml-3 text-bold"><?= round($reviews['rate'], 0)  ?> / 5</h6>
                    </div>
                  </div>
                </div>
                <h6 class="text-capitalize"><?=$reviews['comment'] ?></h6>
