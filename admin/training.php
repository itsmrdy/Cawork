<?php
  ...
  $query = $db->query("SELECT event.title, event.description, event.training_type, event.start_at, event.end_at, reg.firstname, reg.lastname FROM events_db event JOIN reg_db reg ON event.reg_id = reg.id WHERE event.delete_flg = '0' AND event.id = '$event_id';");

  $event_row = mysqli_fetch_assoc($query);

  $event_count = mysqli_num_rows($query);

  $complete_start_at = $event_row['start_at'];

  if ($event_count > 0) {
    $start = explode('T', $event_row['start_at']);
    $end = explode('T', $event_row['end_at']);
  ...
  $js_dateToday = date('Y-m-d').'T'.date('H:i');
    ...
    <section class="content">
            ...
            <div class="card card-dark">
              ...
              ?>
                ...
                <p>Date: <?=$date?><br>From: <?=$start_at?> To: <?=$end_at?></p>
              <?php
                }
              ?>
              </div>
