...
<?php
    ...
    $validate_rating = $db->query("SELECT id FROM ratings WHERE user_id = '$reg_id' AND job_id = '{$service_id}'");
    $validate = mysqli_num_rows($validate_rating);

    if ($validate > 0) {
?>
<script>
    alert('You already add a rating for this client');
