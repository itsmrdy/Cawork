<?php
    ...
    $job_id = $_GET['job_id'];
    $my_id = $_GET['from'];
    if (empty($client_id) || empty($job_id) || empty($my_id)) {
?>
<script>
    history.back();
</script>
<?php
    ...
    $result = $db->query("SELECT * FROM reg_db WHERE id = '$client_id';");
    $user_row = mysqli_fetch_assoc($result);

    $validate_rating = $db->query("SELECT id FROM reviews_db WHERE service_id = '$job_id' AND from_id = '$my_id' AND delete_flg = '0';");
    $validate = mysqli_num_rows($validate_rating);

    if ($validate > 0) {
?>
<script>
    alert('You already add a rating for this worker');
...
<script>
    $('#submit_btn').click(function() {
        ...
        var reg_id = '<?=$client_id?>';

        var request = "ratefree";

        var from_id = '<?=$my_id?>';

        var service_id = '<?=$job_id?>';
